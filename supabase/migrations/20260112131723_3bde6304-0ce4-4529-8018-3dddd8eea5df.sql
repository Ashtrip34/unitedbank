...
-- Fix process_reversal to ALWAYS deduct from recipient (force negative) and deduct net amount (90%)
...
BEGIN
  ...
  -- Calculate reversal amount (full amount sender sent)
  v_reversal_amount := ABS(v_transaction.amount);
  
  -- Net amount recipient received (90% after 10% fee)
  v_net_amount_to_deduct := v_reversal_amount * 0.90;
  ...
  -- FORCE deduct from recipient's account (for internal transfers) - ALWAYS DO THIS
  IF v_transaction.recipient_account IS NOT NULL THEN
    ...
    IF v_recipient_account IS NOT NULL THEN
      -- Force deduct the net amount (90%) from recipient - CAN GO NEGATIVE
      v_recipient_new_balance := v_recipient_account.balance - v_net_amount_to_deduct;
      
      -- FORCE UPDATE - no balance check, can go negative
      UPDATE public.accounts 
      SET balance = v_recipient_new_balance, updated_at = now() 
      WHERE id = v_recipient_account.id;
      
      -- Create a debit transaction for recipient showing negative amount
      INSERT INTO public.transactions (
        ...
        'Payment reversed by sender - funds forcefully deducted',
        'completed',
        'reversal'
      );
    END IF;
  ...
  RETURN json_build_object(
    'success', true, 
    'message', 'Payment reversed successfully. Recipient balance deducted.', 
    'new_balance', v_new_balance,
    'deducted_from_recipient', v_net_amount_to_deduct
  );
END;
$$;

-- Also update internal_transfer to show fee as separate negative transaction
...
BEGIN
  ...
  -- Check sufficient balance - REMOVED to allow negative balance
  ...
  INSERT INTO public.transactions (
    ...
    'Transaction fee (10%) for transfer',
    'completed', 'fee'
  );
  ...
  INSERT INTO public.transactions (
    ...
    'Transfer received from United Bank (10% fee deducted by sender)',
    NULL, NULL, NULL, 'United Bank', 'completed', 'transfer'
  );
